...
if (!empty($_SESSION['cart']))
{
  ...
  while ($row = mysqli_fetch_array ($r, MYSQLI_ASSOC))
  {
    ...
    # Display the row/s:
    echo "<h5>Movie Title: </h5>  {$row['mov_title']}  </td>
  ...
  @ £ {$row['mov_price']} = £ " .number_format ($subtotal, 2). " <form>";
  }
   # Display the total.
  echo '
    ...
    <a href="chckout.php?total='.$total.'"><button type="button" class="btn btn-secondary" role="button">Continue Booking Process</button></a></td>
